...
class Training:
    ...
    def train(self, x: NDArray[np.float64], y: NDArray[np.float64], epochs: int, batch_size: int):
        n_samples = x.shape[0]
        ...
        for epoch in range(epochs):
            ...
            for i in range(0, n_samples, batch_size):
                ...
                print(f"[Epoch {epoch+1}] Batch {i//batch_size+1} Loss: {batch_mean_loss:.6f}")

            epoch_mean_loss = np.mean(epoch_losses)
            print(f"➡️ Epoch {epoch+1}/{epochs} complete - Mean Loss: {epoch_mean_loss:.6f}\n")
